...
import { doc, onSnapshot } from "firebase/firestore"; // onSnapshot ব্যবহার করা হয়েছে রিয়েল-টাইম আপডেটের জন্য
...
export default function OrderDetails() {
  ...
  useEffect(() => {
    ...
    // getDoc এর বদলে onSnapshot ব্যবহার করলে পেমেন্ট হওয়ার সাথে সাথে পেজ রিফ্রেশ ছাড়াই স্ট্যাটাস বদলে যাবে
  ...
  const getStatusDisplay = (status: string) => {
    switch (status) {
      ...
      case "Processing":
        return { icon: <Package className="text-blue-500" size={20} />, label: "Processing", color: "text-blue-500", bg: "bg-blue-50" };
      case "Shipped":
        return { icon: <Truck className="text-indigo-500" size={20} />, label: "On the Way", color: "text-indigo-500", bg: "bg-indigo-50" };
      case "Delivered":
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      ...
      <p className="mt-4 text-slate-400 font-bold uppercase tracking-widest text-xs">Loading Details...</p>
    </div>
  );
      ...
      <div className="bg-white border border-slate-100 rounded-[3rem] shadow-xl overflow-hidden">
        ...
        <div className={`m-8 mb-0 p-6 rounded-3xl border flex items-center gap-4 ${statusInfo.bg} ${statusInfo.color.replace('text', 'border')}/20`}>
          ...
          <div>
            ...
            <p className="text-xs font-medium text-slate-500 italic">
              Placed on {order.createdAt?.toDate().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}
            </p>
          </div>
        </div>
        ...
        <div className="p-8 md:p-12 space-y-12">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-900 text-white p-6 rounded-[2rem] shadow-xl shadow-slate-200 space-y-4">
                <div className="flex justify-between items-center border-b border-white/10 pb-3">
                  ...
                  <span className="text-sm font-black text-emerald-400 uppercase">{order.paymentMethod || "Online Payment"}</span>
                </div>
                <div className="flex justify-between items-center border-b border-white/10 pb-3">
                ...
                <div className="flex justify-between items-center pt-2">
                  <span className="text-xs font-black uppercase italic text-slate-400">Order Total</span>
          ...
          <div>
            <h2 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2">
              <Package size={14} className="text-emerald-500" /> Ordered Items ({order.items?.length || 0})
            </h2>
            <div className="grid grid-cols-1 gap-3">
              {order.items?.map((item: any, index: number) => (
                <div key={index} className="flex justify-between items-center p-5 bg-white border border-slate-100 hover:border-emerald-200 hover:shadow-md transition-all rounded-2xl group">
        ...
        <div className="bg-slate-50 p-8 text-center border-t border-slate-100">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">
            Need help with this order? <span className="text-emerald-500 cursor-pointer hover:underline ml-1">Contact Support</span>
          </p>
        </div>
      </div>
    </div>
