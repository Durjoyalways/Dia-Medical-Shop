...
export default function CartPage() {
 ...
 const handleCheckout = async () => {
    ...
    if (!shippingInfo.name || !shippingInfo.phone || !shippingInfo.address) {
      return alert("Please fill up your shipping details (Name, Phone, Address)!");
    }
    if (!paymentMethod) return alert("Please select a payment method!");
    
    setLoading(true);
    try {
      // ১. ডাটাবেসে তথ্য পাঠানো
      ...
      // ৩. ইউজারকে জানানো এবং রিডাইরেক্ট করা
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {cartItems.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                  ...
                  <div key={idx} className="flex flex-col sm:flex-row justify-between items-center bg-white p-6 rounded-3xl border border-slate-100 shadow-sm gap-4">
                    <div className="flex items-center gap-4 w-full sm:w-auto">
                      <div className="w-16 h-16 bg-emerald-50 rounded-2xl flex items-center justify-center text-emerald-500 shrink-0">
                        {item.imageUrl ? <img src={item.imageUrl} className="w-full h-full object-cover rounded-2xl" /> : <ShoppingBag size={24} />}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-2xl border border-slate-100">
                      <button onClick={() => decrementQuantity(idx)}><Minus size={18} /></button>
                      <span className="font-black">{item.quantity || 1}</span>
                      <button onClick={() => incrementQuantity(idx)}><Plus size={18} /></button>
                    </div>
                    <button onClick={() => removeFromCart(idx)} className="text-red-400 p-2"><Trash2 size={20} /></button>
                  </div>
                ))}
              </div>

              {/* Shipping Form - NEW SECTION */}
            ...
            <div className="space-y-6">
              <div className="bg-[#0f172a] p-8 rounded-[2.5rem] text-white shadow-2xl sticky top-24">
                <h2 className="text-xl font-black mb-6 uppercase text-emerald-400">Checkout</h2>
                
                {/* Payment Methods */}
                <div className="mb-8 space-y-4">
                  <p className="text-[10px] font-black uppercase text-slate-400 text-center">Payment Method</p>
                    ...
                    {paymentOptions.map((option) => (
                      <button
                        ...
                        className={`flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all ${
                          paymentMethod === option.id ? "border-emerald-500 bg-emerald-500/10 text-emerald-400" : "border-slate-800 text-slate-500"
                        }`}
                      >
                        {option.icon}
                        <span className="text-[9px] font-black uppercase">{option.name}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <button 
                  ...
                  disabled={loading}
                  className="w-full bg-emerald-500 text-[#0f172a] py-5 rounded-2xl font-black uppercase tracking-widest hover:bg-emerald-400 transition-all disabled:opacity-50"
                >
          ...
          <div className="text-center py-20 bg-white rounded-[3rem] border border-slate-100">
            <ShoppingBag size={40} className="mx-auto mb-4 text-slate-200" />
            <h2 className="text-xl font-bold">Your cart is empty</h2>
            <Link href="/medicines" className="mt-4 inline-block text-emerald-500 font-bold">Go Shopping</Link>
          </div>
        )}
      </div>
    </div>
